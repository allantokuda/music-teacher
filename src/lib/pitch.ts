...
// { pitch_num: 0, pitch_name: 'C2', fft_index: 49, freq: 65.4 },
// { pitch_num: 1, pitch_name: 'C#2', fft_index: 51, freq: 69.29 },
// { pitch_num: 2, pitch_name: 'D2', fft_index: 55, freq: 73.41 },
// etc.
export const pitches = integer_series.map((i) => {
  ...
  const fft_index = Math.round(freq / SCALE);
  const rounded_freq = Math.round(freq * 100) / 100;
  return { pitch_num, pitch_name, fft_index, freq: rounded_freq } as Pitch;
});

export const pitchIndices = pitches.map((n) => n.fft_index);
...
export function pitchGains(fftGains: number[]) {
  return pitchIndices.map((i) => fftGains[i]);
}
...
export function pitchNumForIndex(fftIndex: number): number | undefined {
  return pitches.find((n) => n.fft_index === fftIndex)?.pitch_num;
}
